...
data = pd.read_csv(r'datasets/Polluter_new_tweets.csv')
print(data)

print(data['Tweet_text'].apply(lambda x: len(x)).mean())
...
import pandas
Polluter_new_tweets=data
Polluter_new_tweets['Class']='1'
Polluter_new_tweets.to_csv("datasets/Clean_polluters_New_tweets.csv", index=False)
Polluter_new_tweets.head()

import pandas
f = pandas.Series(' '.join(data['Tweet_text']).split()).value_counts()[:10]
print(f)
...
# # Loading dataset
data = pd.read_csv(r'datasets/Legitimate_users_new_tweets.csv')
print(data.head())
...
import pandas
Legitimate_users_new_tweets = data
Legitimate_users_new_tweets['Class'] = '0'
Legitimate_users_new_tweets.to_csv("datasets/Clean_legitimate_New_tweets.csv", index=False)
Legitimate_users_new_tweets.head()


all_data = pd.concat([Polluter_new_tweets, Legitimate_users_new_tweets])
# all_data.to_csv("datasets/New_all_data_Pollutors_legitimate.csv", index=False)
# print(all_data)
